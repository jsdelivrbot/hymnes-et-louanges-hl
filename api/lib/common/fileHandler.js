 ...
 */
module.exports.getTemplate = function(filename, extension) {
 ...
 */
function setBreakLine(filename, size) {
...
function breakString(str, size) {
  ...
  return str.substr(0, index) + '\n' +
    breakString(str.substr(index).trim(), size);
}
module.exports.breakString = breakString;
 ...
 * setMaxWidth(['file.md', 'file.txt', 80])
 * @param {Array<string>} args - list of file to modify
 * @return {void}
 */
function setMaxWidth(...args) {
...
module.exports.cli = function(args) {
  if (args.command === 'mdify') {
    if (args.operation) {
      setMaxWidth(...args.options._.slice(1));
    }
    else {
